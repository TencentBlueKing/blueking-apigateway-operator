...
type BkGatewayServiceReconciler struct {
	client.Client
	Logger    logr.Logger
...
// the BkGatewayService object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.10.0/pkg/reconcile
func (r *BkGatewayServiceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	svc := &gatewayv1beta1.BkGatewayService{}
	err := r.Client.Get(ctx, client.ObjectKey{Namespace: req.Namespace, Name: req.Name}, svc)
	if err != nil {
		if k8serrors.IsNotFound(err) {
			// service has been deleted, clean it
			innerErr := r.LifeCycle.CleanServiceDiscovery("BkGatewayService", req)
			if innerErr != nil {
				r.Logger.Error(innerErr, "Clean service discovery failed.", "BkGatewayService", req)
				return ctrl.Result{
		...
		r.Logger.Error(err, "Get BkGatewayService from apiserver failed", "obj", req)
...
func (r *BkGatewayServiceReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&gatewayv1beta1.BkGatewayService{}).
		WithEventFilter(r.servicePredicate()).
		Complete(r)
}

// getPolarisConfigPredicate filter PolarisConfig events
func (r *BkGatewayServiceReconciler) servicePredicate() predicate.Predicate {
	return predicate.Funcs{
		CreateFunc: func(e event.CreateEvent) bool {
			if e.Object.GetNamespace() != r.Namespace {
				return false
			}
			conf, ok := e.Object.(*gatewayv1beta1.BkGatewayService)
			if !ok {
				return false
			}
		...
		UpdateFunc: func(e event.UpdateEvent) bool {
			...
			newconf, okNew := e.ObjectNew.(*gatewayv1beta1.BkGatewayService)
			oldconf, okOld := e.ObjectOld.(*gatewayv1beta1.BkGatewayService)
			if !okNew || !okOld {
				return false
			}
