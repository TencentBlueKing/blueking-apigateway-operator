...
func TestNewAPISIXJsonSchemaValidator(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := NewAPISIXJsonSchemaValidator(tt.version, tt.resource, tt.jsonPath, nil)
			if tt.shouldFail {
				assert.Error(t, err)
			} else {
				assert.NoError(t, err)
...
func TestAPISIXJsonSchemaValidatorValidate(t *testing.T) {
	...
	for _, version := range APISIXVersionList {
		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				validator, err := NewAPISIXJsonSchemaValidator(version, tt.resource, tt.jsonPath, nil)
				assert.NoError(t, err)
