...
var _ = Describe("Validation", func() {
	...
	//	DescribeTable("ValidationFieldError cases", func(expected string, err utils.ValidationFieldError) {
	...
	Describe("ValidationErrorMessage", func() {
		It("io.EOF", func() {
			s := utils.ValidationErrorMessage(io.EOF)
			assert.Equal(GinkgoT(), "EOF, json decode fail", s)
		})

		It("not a field error", func() {
			s := utils.ValidationErrorMessage(errors.New("anError"))
		...
		It("empty", func() {
			s := utils.ValidationErrorMessage(validator.ValidationErrors{})
		...
		Context("fieldError: string", func() {
			...
			It("required", func() {
				...
				msg := utils.ValidationErrorMessage(e)
