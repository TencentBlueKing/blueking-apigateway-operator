...
// BkGatewayStreamResourceReconciler reconciles a BkGatewayStreamResource object
type BkGatewayStreamResourceReconciler struct {
...
// the BkGatewayStreamResource object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.14.4/pkg/reconcile
func (r *BkGatewayStreamResourceReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	logger := log.FromContext(ctx)
	logger.V(1).Info("bk gateway stream resource trigger", "obj", req)
	r.adapater.Reconcile(ctx, req, &gatewayv1beta1.BkGatewayStreamResource{}, logger)
	return ctrl.Result{}, nil
}

// SetupWithManager sets up the controller with the Manager.
func (r *BkGatewayStreamResourceReconciler) SetupWithManager(mgr ctrl.Manager) error {
	...
	return ctrl.NewControllerManagedBy(mgr).
		For(&gatewayv1beta1.BkGatewayStreamResource{}).
		Complete(r)
}
